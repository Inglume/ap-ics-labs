...
public class Shuffler {
  ...
  public static void main(String[] args) {
    ...
    System.out
        .println("Results of " + SHUFFLE_COUNT + " consecutive efficient selection shuffles:");
    int[] values2 = {0, 1, 2, 3};
    for (int j = 1; j <= SHUFFLE_COUNT; j++) {
  ...
  /**
   * Apply a "perfect shuffle" to the argument. The perfect shuffle algorithm splits the deck in
   * half, then interleaves the cards in one half with the cards in the other.
   * 
  ...
  public static void perfectShuffle(int[] values) {
    ...
    int j = 0;
    for (int i = 0; i < (values.length + 1) / 2; i++, j += 2) {
      values[j] = temp[i];
    }
    j = 1;
    for (int i = (values.length + 1) / 2; i < values.length; i++, j += 2) {
      values[j] = temp[i];
    }
  }
